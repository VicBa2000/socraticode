...
const FILES = [
  "AGENTS.md",
  ...(Flag.SOCRATICODE_DISABLE_CLAUDE_CODE_PROMPT ? [] : ["CLAUDE.md"]),
...
function globalFiles() {
  ...
  if (Flag.SOCRATICODE_CONFIG_DIR) {
    files.push(path.join(Flag.SOCRATICODE_CONFIG_DIR, "AGENTS.md"))
  }
  files.push(path.join(Global.Path.config, "AGENTS.md"))
  if (!Flag.SOCRATICODE_DISABLE_CLAUDE_CODE_PROMPT) {
...
export namespace Instruction {
  ...
  export class Service extends Context.Service<Service, Interface>()("@socraticode/Instruction") {}
    ...
    Layer.effect(
      ...
      Effect.gen(function* () {
        ...
        const relative = Effect.fnUntraced(function* (instruction: string) {
          ...
          if (!Flag.SOCRATICODE_CONFIG_DIR) {
            log.warn(
              `Skipping relative instruction "${instruction}" - no SOCRATICODE_CONFIG_DIR set while project config is disabled`,
            )
            return []
          }
          return yield* fs
            .globUp(instruction, Flag.SOCRATICODE_CONFIG_DIR, Flag.SOCRATICODE_CONFIG_DIR)
            .pipe(Effect.catch(() => Effect.succeed([] as string[])))
        })
